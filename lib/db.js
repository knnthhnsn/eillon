...
async function upsertSignup({ email, source, size, productSlug, name, utm = {} }) {
  const query = getSql();
  const rows = await query`
    INSERT INTO waitlist_signups (
      email, product_slug, source, size, name, utm_source, utm_medium, utm_campaign
    )
    VALUES (
      ${email},
      ${productSlug || 'beles'},
...
async function listSignups() {
  const query = getSql();
  return query`
    SELECT email, product_slug, source, size, name, status, created_at, updated_at
