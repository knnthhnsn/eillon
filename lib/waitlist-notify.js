const { getInterestSelection } = require('./product-interest');
...
function buildEmail({
  ...
  const product = interest.productLabel;
  const action = isNew ? 'New waitlist signup' : 'Waitlist signup updated';
  const subject = `${action} — ${selectionLabel || interest.selectionLabel}`;
  const lines = [
    ['Name', name || '—'],
    ['Email', email],
    ['Signed up for', selectionLabel || interest.selectionLabel],
    ['Intent', interest.intentLabel || signupIntent || interest.signupIntent],
    ['Entry point', source],
    ['Page', pagePath || '—'],
...
async function notifyWaitlistSignup(signup) {
  ...
  const from = process.env.RESEND_FROM || 'EILLON <care@eillon.maison>';
